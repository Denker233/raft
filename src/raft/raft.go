...
func (rf *Raft) ChangeState(targetState State) {
	rf.mu.Lock()
	defer rf.mu.Lock()
	if rf.state = targetState { return }
		...
		go func(server int) {
			...
			if rf.sendRequestVote(server, &args, &reply) {
				...
				if reply.VoteGranted && rf.state == Candidate {
					atomic.AddInt32(&voteCount, 1)             // vote self
...
func (rf *Raft) readPersist(data []byte) {
	...
	var logs []LogEntry
	if d.Decode(&currentTerm)!= nil || d.Decode(&votedFor)!= nil || d.Decode(&snapshottedIndex)!= nil || d.Decode(&logs) != nil{
		return -1; // err != nil means there is error
	}
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if args.Term < rf.currentTerm || (rf.votedFor!= NULL && rf.votedFor != args.CandidateId && rf.currentTerm==args.Term) {
	...
	reply.Term, response.VoteGranted = rf.currentTerm, true
...
func (rf *Raft) ticker() {
	for rf.killed() == false {
		...
		case <-rf.electionTimer.C:
			...
			rf.currentTerm += 1
			rf.StartElection()
			rf.electionTimer.ResetReset(randTimeDuration(ElectionTimeoutLower, ElectionTimeoutUpper))
			rf.mu.Unlock()
